...
function PlotViewContent() {
  ...
  useEffect(() => {
    if (selectedStatus === "All") {
      ...
      setFilteredPlots(
        plotView.filter((p) => p?.statusId?.statusName === selectedStatus)
      );
    }
  ...
  const getPlotView = async () => {
    try {
      ...
      if (state.siteId || siteId)
        payload.siteId = role === "AGENT" ? siteId : state.siteId;
  ...
  const handleVisitorSubmit = () => {
    if (!state.statusName || !state.visitorId) {
      toast({
        ...
        description: "Please select Status and Visitor",
        variant: "destructive",
      });
      return;
    }
  ...
  return (
    <div className=" bg-slate-950 min-h-screen p-4 text-slate-100">
      {/* HEADER */}
      <div className="sticky top-0 z-10 bg-slate-950 pb-4 space-y-10 overflow-hidden">
          ...
          <div className="p-4 flex flex-col md:flex-row justify-between gap-4 items-start md:items-center">
            ...
            <div className="hidden md:flex gap-2">
              {uniqueStatusCounts.map((status) => (
                ...
                >
                  {status.name}
                  <span className="ml-2 bg-black/20 px-1.5 rounded-md text-[10px]">
                    {status.count}
                  </span>
                </button>
              ))}
            ...
            <div className="md:hidden w-full ">
              <select
                ...
                className="w-full bg-slate-900 text-white text-sm px-3 py-2 rounded-lg border border-slate-700 focus:outline-none "
              >
            ...
            <motion.div
              ...
              style={{
                background: `linear-gradient(145deg, ${
                  plot.statusId?.colorCode || "#334155"
                }, #1e293b)`,
              ...
              <div className="absolute bottom-[105%] left-1/2 -translate-x-1/2 w-48 bg-slate-950/95 backdrop-blur-md border border-slate-700/50 rounded-lg shadow-2xl p-3 text-xs text-slate-200 opacity-0 group-hover:opacity-100 transition-all duration-200 pointer-events-none z-20 transform translate-y-2 group-hover:translate-y-0">
                <div className="flex items-center gap-2 mb-2 pb-2 border-b border-slate-800">
                  <div
                    ...
                    style={{ background: plot.statusId?.colorCode }}
                  ></div>
                ...
                <div className="grid grid-cols-2 gap-x-2 gap-y-1">
                  ...
                  <span className="text-right text-white">{plot.plotCode}</span>
                  <span className="text-slate-400">Unit:</span>{" "}
                  <span className="text-right text-white truncate">
                    {plot.unitId?.UnitName}
                  </span>
        ...
        >
          <div className="p-6 space-y-3">
            {Object.entries(viewData).map(([key, val]) => (
              <div
                key={key}
                className="flex justify-between border-b border-slate-800 pb-2 last:border-0"
              >
            ...
            <div>
              ...
              <div className="relative">
                ...
                >
                  ...
                  {plotStatuses.map((s) => (
                    <option key={s.PlotStatusName} value={s.PlotStatusName}>
                      {s.PlotStatusName}
                    </option>
                  ))}
                </select>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <select
                  ...
                  onClick={() => {
                    if (visitorData.length === 0) getVisitor();
                  }}
                  onChange={(e) => {
                    const vis = visitorData.find(
                    ...
                    if (vis)
                      dispatch({
                        type: "text",
                        name: "visitorName",
                        value: vis.visitorName || vis["Visitor Name"],
                      });
                  }}
                >
