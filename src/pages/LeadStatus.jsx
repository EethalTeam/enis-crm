...
function LeadStatus() {
    ...
    const deleteLeadStatus = async (_id) => {
    ...
    const handleSearchChange = (value) => {
        ...
        const filtered = leadStatus.filter((row) =>
    ...
    const handleViewClick = (row) => {
        setViewData({
            "LeadStatus Code": row.leadStatusCode,
            "LeadStatus Name": row.leadStatustName,
            "LeadStatus Color": row.leadStatusColor,
            "LeadStatusText Color": row.leadStatusTextColor,
    ...
    // ------------------- VALIDATE -------------------
    const Validate = () => {
        if (!state.leadStatusCode) {
            toast({
                title: "Error",
                description: "Enter LeadStatus Code",
                variant: "destructive",
            });
            return;
        }
        if (!state.leadStatustName) {
            toast({
                title: "Error",
                description: "Enter LeadStatus Name",
                variant: "destructive",
            });
            return;
        }
        if (!state.leadStatusColor) {
            toast({
                title: "Error",
                description: "Enter LeadStatus Background Color",
                variant: "destructive",
            });
            return;
        }

        if (!state.leadStatusTextColor) {
            toast({
                title: "Error",
                description: "Enter LeadStatus Text Color",
                variant: "destructive",
            });
            return;
        }
    ...
    const triggerConfirm = (type) => {
        setConfirmState({
            open: true,
            title: type === "Update" ? "Update LeadStatus?" : "Create LeadStatus?",
    ...
    const handleSubmit = async () => {
        ...
        try {
            if (isEdit) {
                ...
                await createLeadStatus(state);
                toast({
                    title: "Success",
                    description: "LeadStatus created successfully",
                    variant: "success",
                });
            }
            clear();
    ...
    const handleDelete = async (_id) => {
        setLoading(true);
        try {
            await deleteLeadStatus(_id);
            toast({
                title: "Deleted",
                description: "LeadStatus deleted",
                variant: "success",
            });
            getAllLeadStatus();
        } catch (err) {
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="border-b border-slate-700">
                                    <th className="py-3 px-4 text-left">Lead Color</th>
                                    <th className="py-3 px-4 text-left">Lead TextColor</th>
                                    <th className="py-3 px-4 text-left">LeadStatus Code</th>
                                    <th className="py-3 px-4 text-left">LeadStatus Name</th>
                            ...
                            <tbody>
                                {loading && filteredData.length === 0 ? (
                                    ...
                                    filteredData.map((row, index) => (
                                        ...
                                        >
                                            <td className="py-3 px-4"> <input type="color" value={row.leadStatusColor} /></td>
                                            <td className="py-3 px-4"> <input type='color' value={row.leadStatusTextColor} /></td>
                                            <td className="py-3 px-4">{row.leadStatusCode}</td>
                                            <td className="py-3 px-4">{row.leadStatustName}</td>
                                        ...
                                        <CardContent className="p-4 space-y-3">
                                            ...
                                            <div className="flex items-center gap-6">
                                                <div className="flex items-center gap-2">
                                                    <span className="text-xs text-slate-400">Lead Color</span>
                                                    <span
                                                        className="w-5 h-5 rounded border border-slate-500"
                                                        style={{ backgroundColor: row.leadStatusColor }}
                                                    />
                                                </div>
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span
                                                        className="w-5 h-5 rounded border border-slate-500"
                                                        style={{ backgroundColor: row.leadStatusTextColor }}
                                                    />
                                                </div>
                                            </div>
                ...
                >
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                <Label>LeadStatus Color *</Label>
                                <Input type='color'
                                    value={state.leadStatusColor}
                                    onChange={(e) =>
                                        storeDispatch(e.target.value, "leadStatusColor")
                                    }
                                />
                            </div>
                            ...
                            <div>
                                <Label>LeadStatus TextColor *</Label>
                                <Input type='color'
                                    value={state.leadStatusTextColor}
                                    onChange={(e) =>
                                        storeDispatch(e.target.value, "leadStatusTextColor")
                                    }
                                />
                            </div>

                            <div>
                                <Label>LeadStatus Code *</Label>
                                <Input
                                    value={state.leadStatusCode}
                                    onChange={(e) =>
                                        storeDispatch(e.target.value, "leadStatusCode")
                                    }
                                />
                            </div>

                            <div>
                                <Label>LeadStatus Name *</Label>
                                <Input
                                    value={state.leadStatustName}
                                    onChange={(e) =>
                                        storeDispatch(e.target.value, "leadStatustName")
                                    }
                                />
                            </div>
                        </div>
