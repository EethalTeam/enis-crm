...
const visitorVerientReducer = (state, action) => {
  switch (action.type) {
    case "text":
...
// ------------------- EMPLOYEE CONTENT -------------------
function VisitorVerient() {
  const { toast } = useToast();
  const navigate = useNavigate();
  const [visitiorVerient, setVisitiorVerient] = useState([]);
  ...
  const [state, dispatch] = useReducer(visitorVerientReducer, initialState);
  ...
  useEffect(() => {
    if (Permissions.isView) {
      getAllVisitorVerient();
    }
  }, [Permissions]);

  const getAllVisitorVerient = async () => {
    try {
      setLoading(true);
      let url = config.Api + "VisitorVerient/getAllVisitorVariant";
      ...
      setVisitiorVerient(data);
      setFilteredData(data);
    } catch (err) {
      toast({
        title: "Error",
        description: "Could not fetch VisitorVerient",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  // ------------------- CRUD APIs -------------------
  const createVisitorVerient = async (data) => {
    let url = config.Api + "VisitorVerient/createVisitorVerient";
    const res = await fetch(url, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  const updateVisitorVerient = async (data) => {
    let url = config.Api + "VisitorVerient/updateVisitorVerient";
    const res = await fetch(url, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(data),
  ...
  const deleteVisitorVerient = async (_id) => {
    let url = config.Api + "VisitorVerient/deleteVisitorVerient";
    const res = await fetch(url, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  const handleSearchChange = (value) => {
    ...
    const filtered = visitiorVerient.filter((row) =>
  ...
  const handleViewClick = (row) => {
    setViewData({
      "VisitorVerient Code": row.visitorVerientCode,
      "VisitorVerient Name": row.visitorVerientName,
    });
    setViewOpen(true);
  };
  ...
  // ------------------- VALIDATE -------------------
  const Validate = () => {
    if (!state.visitorVerientCode) {
      toast({
        title: "Error",
        description: "Enter Visitor Verient Code",
        variant: "destructive",
      });
      return;
    }
    if (!state.visitorVerientName) {
      toast({
        title: "Error",
        description: "Enter Visitor Verient Name",
        variant: "destructive",
      });
      return;
    }
  ...
  const triggerConfirm = (type) => {
    setConfirmState({
      open: true,
      title:
        type === "Update" ? "Update VisitorVerient?" : "Create VisitorVerient?",
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isEdit) {
        await updateVisitorVerient(state);
        toast({
          title: "Success",
          description: "VisitorVerient updated successfully",
          variant: "success",
        });
      } else {
        await createVisitorVerient(state);
        toast({
          title: "Success",
          description: "VisitorVerient created successfully",
          variant: "success",
        });
      }
      clear();
  ...
  const handleDelete = async (_id) => {
    setLoading(true);
    try {
      await deleteVisitorVerient(_id);
      toast({
        title: "Deleted",
        description: "VisitorVerient deleted",
        variant: "success",
      });
      getAllVisitorVerient();
    } catch (err) {
  ...
  return (
    <div className="space-y-6 bg-slate-950 min-h-screen p-4 text-slate-100">
      ...
      <div className="flex md:flex-row flex-col  items-start space-y-2 md:items-center md:justify-between">
        <h1 className="text-3xl font-bold text-white">Visitor Verient</h1>
        ...
        {Permissions.isAdd && (
          ...
          >
            <Plus className="w-4 h-4 mr-2" /> Add VisitorVerient
          </Button>
        )}
      </div>
        ...
        <CardContent className="p-6">
          ...
          <div className="flex gap-4 mb-6">
            <div className="relative flex-1">
              ...
              <Input
                placeholder="Search Visitor Verient Name..."
                value={searchTerm}
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-700">
                  <th className="py-3 px-4 text-left">VisitorVerient Code</th>
                  <th className="py-3 px-4 text-left">VisitorVerient Name</th>
              ...
              <tbody>
                {loading && filteredData.length === 0 ? (
                  ...
                  filteredData.map((row, index) => (
                    ...
                    >
                      <td className="py-3 px-4">{row.visitorVerientCode}</td>
                      <td className="py-3 px-4">{row.visitorVerientName}</td>
        ...
        <CardContent className="p-6">
          ...
          <div className="flex gap-4 mb-6">
            <div className="relative flex-1">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-fuchsia-400" />
              <Input
                placeholder="Search VisitorVerient Name..."
                value={searchTerm}
                onChange={(e) => handleSearchChange(e.target.value)}
                className="pl-10 bg-purple-900/50 border-fuchsia-700 text-white"
              />
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent
          ...
          title={isEdit ? "Edit Verient" : "Add Verient"}
        ...
        >
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-1 gap-4">
              <div>
                <Label>VisitorVerient Code *</Label>
                <Input
                  value={state.visitorVerientCode}
                  onChange={(e) =>
                    storeDispatch(e.target.value, "visitorVerientCode")
                  }
                />
              </div>

              <div>
                <Label>VisitorVerient Name *</Label>
                <Input
                  value={state.visitorVerientName}
                  onChange={(e) =>
                    storeDispatch(e.target.value, "visitorVerientName")
                  }
                />
              </div>
            </div>
      ...
      <Dialog
        ...
        onOpenChange={() => setConfirmState({ ...confirmState, open: false })}
      >
...
// ------------------- EXPORT -------------------
export default function VisitorVerientPage() {
  return (
    <ToastProvider>
      <VisitorVerient />
    </ToastProvider>
