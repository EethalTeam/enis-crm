...
const VisitorDialog = ({ open, onOpenChange, onSuccess, initialData }) => {
  ...
  useEffect(() => {
    if (open) {
      ...
      fetchAPI("Visitor/getAllEmployees").then((res) => setEmployeeList(res?.data || []));
      fetchAPI("Visitor/getAllStatus", { unitId: state.unitId }).then((res) => setStatusList(res?.data || []));
  ...
  useEffect(() => {
    if (state.siteId) {
      fetchAPI("Unit/getAllUnits", { siteId: state.siteId }).then((res) => setUnitList(res || []));
    } else {
      setUnitList([]);
    }
  }, [state.siteId]);
  ...
  useEffect(() => {
    if (state.unitId) {
      fetchAPI("Visitor/getAllPlots/", { siteId: state.siteId, unitId: state.unitId }).then((res) =>
  ...
  const getCityList = (stateId) => {
    if(!stateId) return;
    fetchAPI("City/getAllCitys", { StateID: stateId }).then((res) => setCityList(res || []));
  }
  ...
  const populateData = (data) => {
    ...
    if (data.cityId) {
        ...
        if (stateData) {
            ...
            getCityList(sId);
            if(typeof stateData === 'object') dispatch({ type: 'text', name: "StateName", value: stateData.StateName });
        }
    }
  ...
  const handleMainSubmit = async () => {
    if (!state.visitorName || !state.visitorMobile || !state.visitorAddress) {
        toast({ title: "Validation Error", description: "Name, Mobile and Address are required", variant: "destructive" });
        return;
    }
    ...
    try {
        ...
        if(res.ok) {
            toast({ title: "Success", description: "Visitor saved successfully", variant: "success" });
            onSuccess();
            onOpenChange(false);
        } else {
        ...
        toast({ title: "Error", description: "Failed to save visitor", variant: "destructive" });
    } finally {
        setLoading(false);
    }
  };
  ...
  const handleSubItemSubmit = async (type) => {
    ...
    try {
        ...
        if(type === 'followup') {
             // Validation
             if(!state.followUpDate || !state.followUpStatus) throw new Error("Date and Status required");
             
             payload = {
                visitorId: state._id, followUpId: state.followUpId,
                followUpDate: state.followUpDate, followedUpById: state.followedUpById,
                followUpStatus: state.followUpStatus, followUpDescription: state.followUpDescription,
                notes: state.notes, remarks: state.remarks
             };
             url = state.followUpId ? "Visitor/updateFollowUp" : "Visitor/addFollowUp";
        } else {
            // Plot Validation
            if(!state.siteId || !state.unitId || !state.statusId) throw new Error("Site, Unit, and Status required");
            
            payload = {
                visitorId: state._id, siteId: state.siteId, unitId: state.unitId, statusId: state.statusId,
                plotIds: [state.plotId], plotId: [state.plotId] // Handling both API naming conventions from your code
            };
            url = state.plotId && plotDetails.find(p => p.plotId._id === state.plotId) ? "Visitor/updateVisitorPlot" : "Visitor/addPlotToVisitor";
        }
        ...
        if(res.ok) {
            toast({ title: "Success", description: `${type === 'followup' ? 'Follow Up' : 'Plot'} Saved`, variant: "success" });
            // Refresh sub-lists
            const updated = await res.json();
            if(type === 'followup') {
                const refresh = await fetchAPI("Visitor/getVisitorFollowUps", { visitorId: state._id });
                ...
                const refresh = await fetchAPI("Visitor/getVisitorPlots", { visitorId: state._id });
  ...
  const editSubItem = (item, type) => {
      if(type === 'followup') {
          ...
          dispatch({ type: "text", name: "followUpDate", value: item.followUpDate?.split('T')[0] });
          dispatch({ type: "text", name: "followUpStatus", value: item.followUpStatus });
          dispatch({ type: "text", name: "notes", value: item.notes });
          dispatch({ type: "text", name: "remarks", value: item.remarks });
          if(item.followedUpById) {
              dispatch({ type: "text", name: "followedUpById", value: item.followedUpById._id });
              dispatch({ type: "text", name: "followedUpByName", value: item.followedUpById.EmployeeName });
          }
      } 
  ...
  const TabButton = ({ id, label, icon: Icon }) => (
    ...
    >
      ...
      {activeTab === id && (
        <motion.div layoutId="activeVisitorTab" className="absolute inset-0 rounded-md bg-fuchsia-600 -z-10" />
      )}
    </button>
  );
      ...
      <DialogContent className="sm:max-w-[850px] max-h-[90vh] flex flex-col p-0">
        <DialogHeader>
          <div className="flex justify-between items-center w-full">
            <DialogTitle>{isEdit ? "Edit Visitor" : "Add New Visitor"}</DialogTitle>
        ...
        <div className="overflow-y-auto p-6 custom-scrollbar flex-1">
          ...
          {activeTab === "contact" && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                ...
                <Input value={state.visitorName} onChange={(e) => storeDispatch(e.target.value, "visitorName", "text")} className="bg-slate-900 border-slate-700 text-white" />
              </div>
              <div className="space-y-2">
                <Label>Mobile *</Label>
                <Input value={state.visitorMobile} onChange={(e) => storeDispatch(e.target.value, "visitorMobile", "text")} className="bg-slate-900 border-slate-700 text-white" />
              </div>
              <div className="space-y-2">
                <Label>Email</Label>
                <Input value={state.visitorEmail} onChange={(e) => storeDispatch(e.target.value, "visitorEmail", "text")} className="bg-slate-900 border-slate-700 text-white" />
              </div>
              <div className="space-y-2">
                <Label>WhatsApp</Label>
                <Input value={state.visitorWhatsApp} onChange={(e) => storeDispatch(e.target.value, "visitorWhatsApp", "text")} className="bg-slate-900 border-slate-700 text-white" />
              </div>
              
              <div className="space-y-2">
                <Label>State</Label>
                <select value={state.StateID} onChange={(e) => {
                     const selected = stateList.find(s => String(s._id) === e.target.value);
              ...
              <div className="space-y-2">
                <Label>City</Label>
                <select value={state.cityId} onChange={(e) => {
                     const selected = cityList.find(c => String(c._id) === e.target.value);
              ...
              <div className="space-y-2">
                ...
                <select value={state.employeeId} onChange={(e) => {
                     const selected = employeeList.find(emp => String(emp._id) === e.target.value);
                     storeDispatch(selected, 'employeeId', 'select');
                }} className="w-full h-10 bg-slate-900 border border-slate-700 rounded-md px-3 text-white">
                     <option value="">Select Employee</option>
                     {employeeList.map(e => <option key={e._id} value={e._id}>{e.EmployeeName}</option>)}
                </select>
              </div>

              <div className="md:col-span-3 space-y-2">
                <Label>Address *</Label>
                <Textarea value={state.visitorAddress} onChange={(e) => storeDispatch(e.target.value, "visitorAddress", "text")} className="bg-slate-900 border-slate-700 text-white h-20" />
              </div>
              <div className="md:col-span-3 space-y-2">
                 <Label>Description</Label>
                 <Textarea value={state.description} onChange={(e) => storeDispatch(e.target.value, "description", "text")} className="bg-slate-900 border-slate-700 text-white" />
              </div>
            </div>
          )}
          ...
          {activeTab === "followup" && (
            <div className="space-y-6">
               ...
               <div className="p-4 bg-slate-900/50 rounded-lg border border-slate-800 space-y-4">
                  <h3 className="text-sm font-bold text-fuchsia-400 uppercase tracking-wider">{state.followUpId ? 'Edit' : 'Add'} Follow Up</h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="space-y-2">
                        <Label>Date</Label>
                        <Input type="date" className="text-white bg-slate-900 border-slate-700" value={state.followUpDate} onChange={(e) => storeDispatch(e.target.value, 'followUpDate', 'text')} />
                      </div>
                      <div className="space-y-2">
                         <Label>Status</Label>
                         <select value={state.followUpStatus} onChange={(e) => {
                             const selected = Status.find(s => s.StatusName === e.target.value);
                             storeDispatch(selected, 'FollowedUpStatus', 'select');
                         }} className="w-full h-10 bg-slate-900 border border-slate-700 rounded-md px-3 text-white">
                            <option value="">Select</option>
                            {Status.map(s => <option key={s.StatusIDPK} value={s.StatusName}>{s.StatusName}</option>)}
                         </select>
                      </div>
                      <div className="space-y-2">
                         <Label>Assign Staff</Label>
                         <select value={state.followedUpById} onChange={(e) => {
                             const selected = employeeList.find(emp => String(emp._id) === e.target.value);
                      ...
                      <div className="md:col-span-3 space-y-2">
                         <Label>Notes</Label>
                         <Input value={state.notes} onChange={(e) => storeDispatch(e.target.value, 'notes', 'text')} className="bg-slate-900 border-slate-700 text-white" />
                      </div>
                  </div>
                  {isEdit && (
                       <Button size="sm" onClick={() => handleSubItemSubmit('followup')} disabled={loading}>
                           {loading && <Loader2 className="w-3 h-3 mr-2 animate-spin" />} Save Follow Up
                       </Button>
                  )}
               </div>
               ...
               <div className="space-y-2">
                  ...
                  {followUpDetails.length === 0 ? <p className="text-sm text-slate-500">No history found.</p> : (
                      <div className="space-y-3">
                          {followUpDetails.map((item, i) => (
                             <div key={i} className="flex justify-between items-start bg-slate-900/40 p-3 rounded-md border border-slate-800">
                                 <div>
                                     <div className="flex items-center gap-2 mb-1">
                                         <Badge variant={item.followUpStatus === 'Pending' ? 'destructive' : 'default'} className={item.followUpStatus === 'Completed' ? 'bg-green-600' : ''}>{item.followUpStatus}</Badge>
                                         <span className="text-xs text-slate-400">{item.followUpDate?.split('T')[0]}</span>
                                     </div>
                                     <p className="text-sm text-slate-200">{item.notes}</p>
                                     <p className="text-xs text-slate-500 mt-1">By: {item.followedUpById?.EmployeeName || 'Unknown'}</p>
                                 </div>
                                 <Button variant="ghost" size="icon" onClick={() => editSubItem(item, 'followup')} className="text-yellow-500 hover:text-yellow-400"><Edit className="w-4 h-4" /></Button>
                             </div>
                          ))}
                      </div>
                  )}
          ...
          {activeTab === "plots" && (
             <div className="space-y-6">
                <div className="p-4 bg-slate-900/50 rounded-lg border border-slate-800 space-y-4">
                  <h3 className="text-sm font-bold text-fuchsia-400 uppercase tracking-wider">Add Plot Interest</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                          <Label>Site</Label>
                          <select value={state.siteId} onChange={(e) => {
                      ...
                      <div className="space-y-2">
                          <Label>Unit</Label>
                          <select value={state.unitId} onChange={(e) => {
                             const selected = unitList.find(u => u._id === e.target.value);
                             storeDispatch(selected, 'unitId', 'select');
                          }} className="w-full h-10 bg-slate-900 border border-slate-700 rounded-md px-3 text-white">
                             <option value="">Select Unit</option>
                             {unitList.map(u => <option key={u._id} value={u._id}>{u.UnitName}</option>)}
                          </select>
                      </div>
                      <div className="space-y-2">
                          <Label>Plot No</Label>
                          <select value={state.plotId} onChange={(e) => {
                             const selected = plotList.find(p => p._id === e.target.value);
                             storeDispatch(selected, 'plotId', 'select');
                          }} className="w-full h-10 bg-slate-900 border border-slate-700 rounded-md px-3 text-white">
                             <option value="">Select Plot</option>
                             {plotList.map(p => <option key={p._id} value={p._id}>{p.plotNumber}</option>)}
                          </select>
                      </div>
                      <div className="space-y-2">
                          <Label>Status</Label>
                          <select value={state.statusId} onChange={(e) => {
                             const selected = statusList.find(s => s._id === e.target.value);
                             storeDispatch(selected, 'statusId', 'select');
                          }} className="w-full h-10 bg-slate-900 border border-slate-700 rounded-md px-3 text-white">
                             <option value="">Select Status</option>
                             {statusList.map(s => <option key={s._id} value={s._id}>{s.statusName}</option>)}
                          </select>
                      </div>
                  </div>
                  {isEdit && (
                       <Button size="sm" onClick={() => handleSubItemSubmit('plot')} disabled={loading}>
                           {loading && <Loader2 className="w-3 h-3 mr-2 animate-spin" />} Save Plot Info
                       </Button>
                  )}
                </div>

                <div className="space-y-2">
                   <h3 className="text-sm font-semibold text-white">Plot History</h3>
                   {plotDetails.length === 0 ? <p className="text-sm text-slate-500">No plots assigned.</p> : (
                           ...
                           <tbody className="text-slate-200">
                               {plotDetails.map((item, i) => (
                                   <tr key={i} className="border-t border-slate-800">
                                       <td className="p-3">{item.plotId?.unitId?.UnitName}</td>
        ...
        <DialogFooter>
           ...
           {(!isEdit || activeTab === 'contact') && (
               <Button onClick={handleMainSubmit} disabled={loading} className="bg-gradient-to-r from-fuchsia-600 to-pink-600 text-white">
                 {loading && <Loader2 className="w-4 h-4 mr-2 animate-spin" />} Save Visitor
               </Button>
           )}
        </DialogFooter>
      </DialogContent>
...
function VisitorContent() {
  ...
  const handleDelete = (row) => {
    Swal.fire({
      title: 'Are you sure?', text: 'Do you really want to delete this data?', icon: 'warning',
  ...
  return (
    <div className="space-y-6 bg-slate-950 min-h-screen p-4 mt-3 text-slate-100">
      <Helmet><title>Visitors - CRM</title></Helmet>
      ...
      <div className="flex md:flex-row flex-col items-start md:justify-between gap-3">
        ...
        <div className="flex gap-3">
          ...
          <Button
            onClick={() => { setSelectedVisitor(null); setDialogOpen(true); }}
      ...
      <Card className="bg-slate-900 border-slate-800">
        <CardContent className="p-6">
          <div className="flex gap-4 mb-6">
            ...
            <Button variant="ghost" onClick={fetchVisitors} className="text-fuchsia-300">
            ...
            <table className="w-full text-left">
              ...
              <tbody>
                {filteredVisitors.map((row) => (
                  <tr key={row._id} className="border-b border-slate-800 hover:bg-slate-800/50 transition-colors group">
                    <td className="py-3 px-4 text-slate-300 text-sm">{row.visitorCode || 'N/A'}</td>
                    <td className="py-3 px-4 font-medium text-white">
                        <span onClick={() => { setSelectedVisitor(row); setDialogOpen(true); }} className="cursor-pointer underline-offset-4 group-hover:underline text-fuchsia-400">
                             {row.visitorName}
                        </span>
                    </td>
                    <td className="py-3 px-4 text-slate-300">{row.visitorMobile}</td>
                    <td className="py-3 px-4 text-slate-300">{row.visitorEmail}</td>
                    <td className="py-3 px-4 flex justify-end gap-2">
                      <Button size="icon" variant="ghost" onClick={() => { setSelectedVisitor(row); setDialogOpen(true); }} className="text-yellow-400 hover:bg-yellow-400/10">
                        <Edit className="w-4 h-4" />
                      </Button>
                      <Button size="icon" variant="ghost" onClick={() => handleDelete(row)} className="text-red-400 hover:bg-red-900/20">
          ...
          <div className="space-y-4 md:hidden">
            {filteredVisitors.map((row) => (
              <div key={row._id} className="rounded-xl border border-slate-700 bg-slate-900 shadow-md p-5 space-y-3">
                 <div className="flex justify-between items-start">
                     <div>
                         <p className="text-xs text-slate-400">Code: {row.visitorCode}</p>
                         <h3 className="text-lg font-bold text-fuchsia-400" onClick={() => { setSelectedVisitor(row); setDialogOpen(true); }}>{row.visitorName}</h3>
                     </div>
                 </div>
                 <div className="text-sm space-y-1">
                     <div className="flex items-center gap-2 text-slate-300"><Phone className="w-4 h-4" /> {row.visitorMobile}</div>
                     <div className="flex items-center gap-2 text-slate-300"><Mail className="w-4 h-4" /> {row.visitorEmail}</div>
                 </div>
                 <div className="flex justify-end gap-3 pt-3 border-t border-slate-800">
                      <Button size="icon" variant="ghost" onClick={() => { setSelectedVisitor(row); setDialogOpen(true); }} className="text-yellow-400">
                        <Edit className="w-5 h-5" />
                      </Button>
                      <Button size="icon" variant="ghost" onClick={() => handleDelete(row)} className="text-red-400">
